...
class Aplicacion:
    ...
    def guardarTexto(self):
        ...
        self.ventana.config(cursor="crosshair")
    ...
    def botonPresionado(self,btn):
        ...
        if(btn == "punto"):
            ...
            self.ventana.config(cursor="target")
        elif(btn == "borrar"):
            self.ventana.config(cursor="dot")
        elif(btn == "colores"):
    ...
    def btnCordenadas(self):
        ...
        if(self.boton == "seleccion"):
            self.PosicionIncialFigura(cordenadas)

        if(self.boton == "mover"):
            print("Las cordenadas X:{0} y las Y:{1}".format(cordenadas.x,cordenadas.y))
            self.crearFigura(cordenadas)
        ...
        if(self.boton == "seleccion"):
            ...
            for i in range(self.contadorFiguras):
                if(self.figura[i].seleccionar(event.x,event.y)):
                    ...
                    self.figuraSeleccionada = i
                    return 1
            self.figuraSeleccionada = -1
            return 0

        if(self.boton == "mover"):
            if(self.figuraSeleccionada != -1):
                 self.figura[self.figuraSeleccionada].borrarFigura(self.lienzoCanvas)
    ...
    def dibujarFigura(self,event):
        ...
        if(self.boton == "mover"):
            if(self.figuraSeleccionada != -1):
                if("Punto.Texto" not in str(type(self.figura[self.figuraSeleccionada]))):
                     self.figura[self.figuraSeleccionada].dibujarMovido(self.lienzoCanvas,event.x,event.y,self.figura[self.figuraSeleccionada].co())
                     self.figura[self.figuraSeleccionada].dibujarMovido(self.lienzoCanvas,self.ultimaX,self.ultimaY,"white")
                else:
                    self.figura[self.figuraSeleccionada].dibujarMovido(self.lienzoCanvas,event.x,event.y,self.color)
                    self.figura[self.figuraSeleccionada].dibujarMovidoRectangulo(self.lienzoCanvas,self.ultimaX,self.ultimaY,"white")
                    import time
                    time.sleep(0.09)
    ...
    def crearFigura(self,event):
        ...
        if(self.boton == "mover"):
            if(self.figuraSeleccionada != -1):
                self.figura[self.figuraSeleccionada].moverObjeto(event.x,event.y)
                self.figura[self.figuraSeleccionada].dibujar(self.lienzoCanvas)
                self.dibujarTodo()
                self.figuraSeleccionada = -1
