...
class Aplicacion:
    def __init__(self, ventanaPrincipal):
        ...
        self.ventana.bind("<Key>", self.teclaPresionada) # Evento que captura las teclas en la ventana
        self.ventana.bind("<BackSpace>", self.teclaBorrar) # Evento que captura la tecla retroceso
        self.ventana.bind("<space>", self.teclaEspacio) # Evento que captura la tecla espacio
        self.ventana.bind("<Delete>", self.teclaBorrar) # Evento que captura la tecla suprimir

        self.ventana.bind("<Control-s>", lambda event,tecla="seleccion":self.teclaCombinadas(event,tecla)) # Evento que captura la tecla ctrl+s
        self.ventana.bind("<Control-m>", lambda event,tecla="mover":self.teclaCombinadas(event,tecla)) # Evento que captura la tecla ctrl+m
        self.ventana.bind("<Control-c>", lambda event,tecla="copiar":self.teclaCombinadas(event,tecla)) # Evento que captura la tecla ctrl+c
        self.ventana.bind("<Control-r>", lambda event,tecla="colores":self.teclaCombinadas(event,tecla)) # Evento que captura la tecla ctrl+c

        # Creacion de variables importantes que se necesitaran a lo largo del codigo

        self.botonAnterior = "seleccion"
        self.boton = "seleccion" # Variable que guardara el boton que este presionado, por defecto seleccionar
        ...
        self.imagenDescargar = PhotoImage(file='descargar.png')
        self.botonDescargar = Button(self.frameCoordenados,image=self.imagenDescargar,height=50,width=50,borderwidth=3, relief="groove",bg="pale green",command=lambda:self.botonPresionado("descargar"))
        self.botonDescargar.place(x=440,y=212)

        self.botonDescargar.bind("<Enter>", lambda event, btn="Descargar archivo",X=1220,Y=640 : self.adentro(event,btn,X,Y))
        self.botonDescargar.bind("<Leave>", self.afuera)
    ...
    def teclaCombinadas(self,event,tecla): # Funcion cuando se presiona una tecla + control
        self.botonPresionado(tecla) # simula como si una tecla se presionara, entrando en la funcion del boton

    def teclaBorrar(self,event): # Funcion cuando se presiona la tecla borrar o suprimir
        self.botonPresionado("borrar") # simula como si una tecla se presionara, entrando en la funcion del boton

    def teclaPresionada(self,event): # Funcion cuando se presiona la tecla "ordinaria"
        tecla = repr(event.char) # Se hace una representacion de la letra presionada
        # Esto le añade '' a la tecla presionada

        if(tecla[1:-1] == "p" or tecla[1:-1] == "P"):
            self.botonPresionado("punto") # simula como si una tecla se presionara, entrando en la funcion del boton
        elif(tecla[1:-1] == "l" or tecla[1:-1] == "L"):
            self.botonPresionado("linea") # simula como si una tecla se presionara, entrando en la funcion del boton
        elif(tecla[1:-1] == "r" or tecla[1:-1] == "R"):
            self.botonPresionado("recta") # simula como si una tecla se presionara, entrando en la funcion del boton
        elif(tecla[1:-1] == "c" or tecla[1:-1] == "C"):
            self.botonPresionado("circulo") # simula como si una tecla se presionara, entrando en la funcion del boton
        elif(tecla[1:-1] == "t" or tecla[1:-1] == "T"):
            self.botonPresionado("trian") # simula como si una tecla se presionara, entrando en la funcion del boton
        elif(tecla[1:-1] == "q" or tecla[1:-1] == "Q"):
            self.botonPresionado("texto") # simula como si una tecla se presionara, entrando en la funcion del boton

    def teclaEspacio(self,event):
        print("Espacio")
        self.botonPresionado(self.botonAnterior)
    ...
    def ventanaOpcionTexto(self): # Creacion de la ventana que permite ingresar texto usuario
        ...
        self.ventanaTexto.geometry("420x260+400+130") # Se le da un tamaño de 400x260 pixeles, ademas cuando se le suma se corre 400 pixeles a la derecha y 130 pixeles abajo
        ...
        self.entryTamano.insert(END,"1")

        self.labelPx = Label(self.frame1,text="x20 px",font=("Calibri",14))
        self.labelPx.place(x=180,rely=0.5, anchor=CENTER)
        ...
        self.entryTexto = Text(self.frame2,height=4,width=30,state=NORMAL,font=("Courier New",12))
    ...
    def ventanaDescargarCodigo(self):
        self.botonDescargar.config(state=DISABLED,bg=(self.ventana.cget('bg'))) # Cuando se abre la ventana se desabilita el boton descarga, esto para que no se abra mas de una ventana
        self.ventanaDescarga = Toplevel(self.ventana) # La ventanaDescarga hereda de la ventana principal
        self.ventanaDescarga.transient(self.ventana) # Como la vventanaDescarga hereda de la ventana principal, entonces hasta que no se cierre la ventanaDescarga no se puede quitar
        self.ventanaDescarga.geometry("350x100+950+590") # Se le da un tamaño de 350x100 pixeles, ademas cuando se le suma se corre 950 pixeles a la derecha y 590 pixeles abajo
        self.ventanaDescarga.resizable(False,False) # No deja redimensionar la pantalla
        self.ventanaDescarga.title("Descargar codigo") # Configura el titulo de la ventana

        self.entryDireccionAlma = Entry(self.ventanaDescarga,font=("Arial Narrow",13),fg="black",bg="white",justify="right",width=41,state="normal")
        self.entryDireccionAlma.place(y=20,relx=0.5, anchor=CENTER)

        self.botonAceptarDescarga = Button(self.ventanaDescarga,text="Descargar",command=self.descargarElCodigo,font=("Calibri",14),borderwidth=3,relief="groove") # Boton aceptar, es el encargado de cerrar la ventana y descargar el archivo en la direccion
        self.botonAceptarDescarga.place(relx=0.5,y=70, anchor=CENTER)

        import os # Importamos la libreria os, la cual nos permitira saber la direccion donde se almacena el programa
        direccion = os.path.dirname(os.path.abspath(__file__)) # Comando que permite saber donde esta almacenado el archivo
        direccion += "\Disenos\SinTitulo.txt" # Le añadimos a la direccion la carpeta y el nombre por defecto

        self.entryDireccionAlma.insert(0,direccion) # Ponemos esto en el entry


        self.ventanaDescarga.protocol("WM_DELETE_WINDOW", self.borrarVentanaDescarga) # Evento que se ejecuta cuando se presiona el boton de cerrar ventana


    def descargarElCodigo(self): # Metodo que mapea las figuras y las convierte en codigo
        self.botonDescargar.config(state=ACTIVE) # Activa el boton de descargar que fue deshabilitado al abrir la pestaña
        direccion = self.entryDireccionAlma.get() # Direccion almacena el directorio en el cual se almacenara el archivo, se captura antes de destruir la ventana
        self.ventanaDescarga.destroy() # Cierra la ventana


        archivoCodigo = file = open(direccion, "w") # Crea un archivo de texto en la direccion seleccionada
        archivoCodigo.write("void disenoApp(){ \n")
        ...
        for x in self.dictFiguras.keys(): # Bucle que recorre las claves del diccionario, es decir, (punto,circulo,linea,etc)
                if(x == "punto"): # Si la clave es un punto
                    for i in self.dictFiguras[x]: # i toma los valores de los indices que son un punto
                        if(self.figura[i].x1() != 0 and self.figura[i].y1() != 0 and self.figura[i].x2() != 0 and self.figura[i].y2() != 0):
                            x1Conver = self.convertir(self.figura[i].x1(),0,803,0,320)
                            y1Conver = self.convertir(self.figura[i].y1(),0,603,0,240)
                            color = self.figura[i].co().upper()
                            archivoCodigo.write("    tft.fillCircle({0},{1},{2},{3});\n".format(x1Conver,y1Conver,"1",color))
                    archivoCodigo.write("\n")
                    ...
                    for i in self.dictFiguras[x]: # i toma los valores de los indices que son una linea
                        if(self.figura[i].x1() != 0 and self.figura[i].y1() != 0 and self.figura[i].x2() != 0 and self.figura[i].y2() != 0):
                            x1Conver = self.convertir(self.figura[i].x1(),0,803,0,320)
                            y1Conver = self.convertir(self.figura[i].y1(),0,603,0,240)
                            x2Conver = self.convertir(self.figura[i].x2(),0,803,0,320)
                            y2Conver = self.convertir(self.figura[i].y2(),0,603,0,240)
                            color = self.figura[i].co().upper()
                            archivoCodigo.write("    tft.drawLine({0},{1},{2},{3},{4});\n".format(x1Conver,y1Conver,x2Conver,y2Conver,color))
                    archivoCodigo.write("\n")
                    ...
                    for i in self.dictFiguras[x]: # i toma los valores de los indices que son un rectangulo
                        if(self.figura[i].x1() != 0 and self.figura[i].y1() != 0 and self.figura[i].x2() != 0 and self.figura[i].y2() != 0):
                            ...
                            largo = x2Conver - x1Conver
                            ancho = y2Conver - y1Conver
                            archivoCodigo.write("    tft.drawRect({0},{1},{2},{3},{4});\n".format(x1Conver,y1Conver,largo,ancho,color))
                    archivoCodigo.write("\n")
                    ...
                    for i in self.dictFiguras[x]: # i toma los valores de los indices que son un circulo
                        if(self.figura[i].x1() != 0 and self.figura[i].y1() != 0 and self.figura[i].x2() != 0 and self.figura[i].y2() != 0):
                            ...
                            r1 = (x2Conver - x1Conver) /2
                            r2 = (y2Conver - y1Conver) /2

                            centroX = ((x2Conver-x1Conver) / 2) + x1Conver
                            centroY = ((y2Conver-y1Conver) / 2) + y1Conver

                            #archivoCodigo.write("    tft.drawEllipse({0},{1},{2},{3},{4});\n".format(x1Conver,y1Conver,r1,r2,color))
                            if(r1 >= r2):
                                archivoCodigo.write("    tft.drawCircle({0},{1},{2},{3});\n".format(centroX,centroY,r1,color))
                            else:
                                archivoCodigo.write("    tft.drawCircle({0},{1},{2},{3});\n".format(centroX,centroY,r2,color))
                    ...
                    for i in self.dictFiguras[x]: # i toma los valores de los indices que son un triangulo
                        if(self.figura[i].x1() != 0 and self.figura[i].y1() != 0 and self.figura[i].x2() != 0 and self.figura[i].y2() != 0):
                            ...
                            archivoCodigo.write("    tft.drawLine({0},{1},{2},{3},{4});\n".format(x1Conver,y1Conver,x2Conver,y2Conver,color)) # HIPOTENUSA # crea la figura con los datos que tiene el objeto[i]
                            archivoCodigo.write("    tft.drawLine({0},{1},{2},{3},{4});\n".format(x1Conver,y1Conver,x2Conver,y1Conver,color)) # BASE # crea la figura con los datos que tiene el objeto[i]
                            archivoCodigo.write("    tft.drawLine({0},{1},{2},{3},{4});\n".format(x2Conver,y1Conver,x2Conver,y2Conver,color)) # ALTURA # crea la figura con los datos que tiene el objeto[i]
                    archivoCodigo.write("\n")
                elif(x == "texto"): # Si la clave es un texto
                    for i in self.dictFiguras[x]: # i toma los valores de los indices que son un texto
                        if(self.figura[i].x1() != 0 and self.figura[i].y1() != 0 and self.figura[i].tex()[:-1] != ""):
                            x1Conver = self.convertir(self.figura[i].x1(),0,803,0,320)

                            y1Conver = self.convertir(self.figura[i].y1(),0,603,0,240)
                            tamanoTexto = int(self.figura[i].ttex())/20
                            color = self.figura[i].co().upper()

                            cadena = ""
                            contadorSaltos = y1Conver
                            for k in self.figura[i].tex()[:-1]:
                                cadena += k
                                if (k == "\n"):
                                    archivoCodigo.write("    tft.setCursor({0},{1});\n".format(x1Conver+ (tamanoTexto*2),contadorSaltos+ (tamanoTexto*2)))
                                    archivoCodigo.write("    tft.setTextSize({0});\n".format(tamanoTexto))
                                    archivoCodigo.write("    tft.setTextColor({0});\n".format(color))
                                    archivoCodigo.write("    tft.println(\"{0}\");\n".format(cadena[:-1]))
                                    contadorSaltos += (tamanoTexto*10) 
                                    print(cadena[:-1])
                                    cadena = ""
    ...
    def guardarTexto(self): # Metodo que almacena el texto ingresado por el usuario
        ...
        self.tamanoLetraUsuario = int(self.entryTamano.get())*20 # Se guarda el tamañano de la fuente
    ...
    def borrarVentanaDescarga(self): # Metodo que borra la ventana de descarga
        try: # intenta cerrar la ventana, en algunos casos puede mandar error
            self.botonDescargar.config(state=ACTIVE) # Activa el boton de descargar que fue deshabilitado al abrir la pestaña
            self.ventanaDescarga.destroy() # Cierra la ventana
        except:
    ...
    def botonPresionado(self,btn): # Metodo que se ejecuta cuando se presiona un boton de las herramientas o figuras
        ...
        elif(btn == "descargar"): # Se ejecuta si se presiona el boton descargar
            self.ventanaDescargarCodigo() # Este metodo abre una ventana para escoger la direccion de almacenamiento del archivo
            self.ventana.config(cursor="") # Deja el cursor como estaba de serie
    ...
    def adentro(self,event,btn,X,Y): # Este metodo se ejecuta cuando el cursor esta adentro de un boton
        ...
        elif(btn[:2] == "De"):# Esto impide que se coloree cuando el boton esta deshabilitado, esto cuando se abre una ventana extra
            if(self.botonDescargar.cget('state') == 'normal'):
                self.botonDescargar.config(bg="#ADD8E6")
    ...
    def afuera(self,event): # Este metodo se ejecuta cuando el cursor esta afuera de un boton
        ...
        self.botonDescargar.config(bg="pale green")
    ...
    def dibujarFigura(self,event): # Paso 2  -----> de creacion de las figuras, este boceta las figuras con unas cordenadas iniciales ya dadas, varia en las finales, hasta que el usuario finalice
        if(self.boton == "punto"): # Si es un punto no hace nada, ya que un punto no se tiene que mover
            ...
            self.lienzoCanvas.delete("linea") #Borra la linea creada anteriormente, esto con el fin de visualizr la figura en tiempo real
            self.lienzoCanvas.create_line(self.primeraX,self.primeraY,event.x,event.y,fill=self.color, tag="linea") # Dibuja una linea con las cordenadas iniciales, y con las actuales del mouse
            self.delay() #Demora de 30 ms, esto para poder visualizar la "sucesion" de lineas
        elif(self.boton == "recta"): # Si es un rectangulo
            self.lienzoCanvas.delete("rectangulo") #Borra el rectangulo creado anteriormente, esto con el fin de visualizr la figura en tiempo real
            self.lienzoCanvas.create_rectangle(self.primeraX,self.primeraY,event.x,event.y,outline=self.color, tag="rectangulo") # Dibuja un rectangulo con las cordenadas iniciales, y con las actuales del mouse
            self.delay() #Demora de 30 ms, esto para poder visualizar la "sucesion" de rectangulos   
        elif(self.boton == "circulo"): # Si es un circulo
            self.lienzoCanvas.delete("circulo") #Borra el circulo creado anteriormente, esto con el fin de visualizr la figura en tiempo real
            self.lienzoCanvas.create_oval(self.primeraX,self.primeraY,event.x,event.y,outline=self.color,tag="circulo") # Dibuja un circulo con las cordenadas iniciales, y con las actuales del mouse
            self.delay() #Demora de 30 ms, esto para poder visualizar la "sucesion" de circulos   
        elif(self.boton == "trian"): # Si es un triangulo
            # Dibuja un triangulo rectangulo a partir de tres lineas
            self.lienzoCanvas.delete("triangulo") #Borra el triangulo creado anteriormente, esto con el fin de visualizr la figura en tiempo real

            self.lienzoCanvas.create_line(self.primeraX,self.primeraY,event.x,event.y,fill=self.color,tag="triangulo") # HIPOTENUSA  # Dibuja un triangulo con las cordenadas iniciales, y con las actuales del mouse
            self.lienzoCanvas.create_line(self.primeraX,self.primeraY,event.x,self.primeraY,fill=self.color,tag="triangulo") # BASE  # Dibuja un triangulo con las cordenadas iniciales, y con las actuales del mouse
            self.lienzoCanvas.create_line(event.x,self.primeraY,event.x,event.y,fill=self.color,tag="triangulo") # ALTURA # Dibuja un triangulo con las cordenadas iniciales, y con las actuales del mouse

            self.delay() #Demora de 30 ms, esto para poder visualizar la "sucesion" de triangulos   

        elif(self.boton == "texto"): # Si es un texto
            self.lienzoCanvas.delete("texto") #Borra el triangulo creado anteriormente, esto con el fin de visualizr la figura en tiempo real
            self.lienzoCanvas.create_text(event.x,event.y,text=self.textoUsuario,font=("Courier New",self.tamanoLetraUsuario),fill=self.colorTextoUsuario,anchor="nw",tag="texto") # Dibuja un texto con las cordenadas iniciales, y con las actuales del mouse y con el texto ingresado por el usuario anteriormente
            self.delay() #Demora de 30 ms, esto para poder visualizar la "sucesion" de textos   
        ...
        if(self.boton == "mover" or self.boton == "copiar"): # Si se va a copiar o a mover
            if(self.figuraSeleccionada != -1): # Si ya hay una figura previamente seleccionada
                self.lienzoCanvas.delete("mover") #Borra el objeto creado anteriormente, esto con el fin de visualizr la figura en tiempo real
                ...
                self.delay() #Demora de 30 ms, esto para poder visualizar el movimiento de figuras   
    ...
    def crearFigura(self,event):  # Paso 3  -----> de creacion de las figuras, crea el objeto con las cordenadas iniciales almacenadas en el paso 1, y con las cordenadas finales donde el mouse se levanta
        ...
        if(self.boton == "punto"): # Si es un punto
            ...
            self.lienzoCanvas.delete("linea") #Elimina el ultimo objeto que fue creado de la sucesion
            ...
            self.lienzoCanvas.delete("rectangulo") #Elimina el ultimo objeto que fue creado de la sucesion
            ...
            self.lienzoCanvas.delete("circulo") #Elimina el ultimo objeto que fue creado de la sucesion
            ...
            self.lienzoCanvas.delete("triangulo") #Elimina el ultimo objeto que fue creado de la sucesion
            ...
            self.lienzoCanvas.delete("texto") #Elimina el ultimo objeto que fue creado de la sucesion
    ...
    def dibujarTodo(self): # Metodo que dibuja todos los elementos de nuevo en el lienzo
        ...
        for x in self.dictFiguras.keys(): # Bucle que recorre las claves del diccionario, es decir, (punto,circulo,linea,etc)
                if(x == "punto"): # Si la clave es un punto
                    ...
                    for i in self.dictFiguras[x]: # i toma los valores de los indices que son un texto
                        self.lienzoCanvas.create_text(self.figura[i].x1(),self.figura[i].y1(),text=self.figura[i].tex(),font=("Courier New", self.figura[i].ttex()),fill=self.figura[i].co(),anchor="nw") # crea la figura con los datos que tiene el objeto[i]
    # ---------------------------------------- FIN DE CREACION DE FIGURAS -------------------------------------------------

    def delay(self): # Funcion que hace un retraso minimo para poder visualizar la creacion de las figuras en tiempo real 
        from time import sleep
        sleep(0.03) 
        
    def convertir(self,valor, desdeMinimo, desdeMaximo, hastaMinimo, hastaMaximo): # METODO EXTRAIDO DE STACK OVERFLOW AUTOR: Adam Luchjenbroers

        # Figure out how 'wide' each range is
        diferenciaDesde = desdeMaximo - desdeMinimo
        diferenciaHasta = hastaMaximo - hastaMinimo

        # Convert the left range into a 0-1 range (float)
        valorEscalado = float(valor - desdeMinimo) / float(diferenciaDesde)

        # Convert the 0-1 range into a value in the right range.
        return round(hastaMinimo + (valorEscalado * diferenciaHasta))
